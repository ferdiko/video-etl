...
class TransMOT(nn.Module):
    ...
    def forward(self, enc_feature, enc_edge_weight, enc_mask, dec_feature, dec_edge_weight, dec_mask, debug=False):
        ...
        spatial_enc_output = self.spatial_encoder(enc_feature, mask=enc_edge_weight,
                                                  src_key_padding_mask=~enc_mask)  # , enc_edge_weight)
        ...
        dec_mask = dec_mask.to(torch.bool)  # .transpose(0, 1)
        # spatial_enc_output = spatial_enc_output.transpose(0, 1)
        ...
        output = self.decoder(dec_feature.transpose(0, 1), temporal_enc_output, tgt_mask=dec_edge_weight,
                              memory_key_padding_mask=new_enc_mask,
                              tgt_key_padding_mask=new_dec_mask)
        # if debug:
        # print("decoder_out", output)
        return output
